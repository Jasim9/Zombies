...
function ApplyPickup (player : FPSPlayer) {
	if (pickupType == PickupType.Health) {
		...
		var launcher : RocketLauncher = player.GetComponentInChildren(RocketLauncher); // beacuse Rocket Launcher active only when equipped
		if (launcher){
			launcher.ammoCount += amount;
			Debug.Log("Launcher found ammo added");
		}
	}
	return true;
}
